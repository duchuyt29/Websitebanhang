				...
				<div class="col-md-12">
					<form:form
						...
						<div class="form-input">
							...
							<form:input path="create_date" value="${item.create_date }"
								type="date" class="form-control" />
						</div>
						<div class="form-input">
							<label class="form-label">Address :</label>
							<form:input path="address" value="${item.address }"
								class="form-control" />
						</div>
						<div class="form-input">
							<label>User order :</label>
							<form:select path="user_id" value="${item.user_id }"
								class="form-select">
						...
						<div class="form-input m-3">
							...
							<a href="/IT16308_Assignment/admin/orders/create"
								class="btn btn-success">Tạo đặt hàng</a>
						</div>
					</form:form>
				</div>
				<br>
							...
							<c:forEach items="${data.content }" var="order">
								<tr>
									...
									<td><a class="btn btn-primary"
										href="/IT16308_Assignment/admin/orders/edit/${order.id }">Update</a>
									</td>
									<td><button type="button" class="btn btn-danger"
											data-bs-toggle="modal" data-bs-target="#p${order.id}">Delete</button>
										<!-- Modal -->
										<div class="modal fade" id="p${order.id }" tabindex="-1"
											...
											<div class="modal-dialog">
												<div class="modal-content">
													...
													<div class="modal-body">Bạn có chắc chắn muốn xóa đơn
														hàng này không?</div>
													<div class="modal-footer">
														<button type="button" class="btn btn-secondary"
															data-bs-dismiss="modal">Hủy</button>
														<a class="btn btn-danger"
															href="/IT16308_Assignment/admin/orders/delete/${ order.id }">Delete</a>
													</div>
												</div>
											</div>
										</div></td>
